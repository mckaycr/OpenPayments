...
function check(callback){
	var x = new Date(getDate());
		var y = new Date(def.ver_ctrl.updated)
		if(x>y){
			callback(true)
		}else{
			callback(false)
		...
		request.query()
			...
			.getRows()
				.on('success',function(rows){
					organize_datasets(def,rows, function(results){
						callback(results)
					})
				})
...
function organize_datasets(original,rows, callback){
	...
	rows.forEach(function(row){
		...
		if(types.includes(type)){
			...
			if(defs.datasets.hasOwnProperty(type)){
				defs.datasets[type][year]=row.dataset_id
			}else{
				defs.datasets[type] = {[year]:row.dataset_id}
			}
		}else{
			...
			//if(datasets.hasOwnProperty(type)){
				defs.providers[type]=row.dataset_id
			//}else{
			//	datasets = {[type]:row.dataset_id}
			//}
		}
		if(defs.hasOwnProperty('ver_ctrl')){
			defs.ver_ctrl.updated = getDate()
		}else{
			defs.ver_ctrl = {updated:getDate()}
		}
	})
