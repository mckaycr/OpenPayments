            ...
            for (var prop in obj) {
                if(entity_type!='hospital'||prop!='ownership'){
                    if(obj.hasOwnProperty(prop)){
                        switch(entity_type){
                            case 'company':
                                output.push({id:entityObj.id,'year':key,'type':prop,'uri':def.url+obj[prop]+'?$where=applicable_manufacturer_or_applicable_gpo_making_payment_id=%27'+entityObj.id+'%27'});
                                break;
                            default:
                                output.push({id:entityObj.id,'year':key,'type':prop,'uri':def.url+obj[prop]+def.supplement.params[entity_type] + '%27'+entityObj.id+'%27'});
                        }
                    }
                }else if(entity_type=='physician'&&prop=='pi'){
                    if(obj.hasOwnProperty(prop)){
                        output.push({id:entityObj.id,'year':key,'type':prop,'uri':def.url+obj[prop]+"$where=(physician_profile_id IS NULL OR physician_profile_id!=%27"+entityObj.id+"%27)%20AND%20(principal_investigator_1_profile_id=%27"+entityObj.id+"%27%20OR%20principal_investigator_2_profile_id=%27"+entityObj.id+"%27%20OR%20principal_investigator_3_profile_id=%27"+entityObj.id+"%27%20OR%20principal_investigator_4_profile_id=%27"+entityObj.id+"%27%20OR%20principal_investigator_5_profile_id=%27"+entityObj.id+"%27"});
                    }   
                }
            }
        }       
