...
function Covered_Recipient(){
  ...
  //   	if(typeof callback !=='function'){
	 ...
	 //    				query.offset = query.offset+100000
	 //    				i++
	 //    			}else{
		//     			itemsProcessed++;
		//     			if(itemsProcessed===queries.length){
...
function query(entity, callback){
	...
	var options = {
		username:'christopher.mckay@cms.hhs.gov',
		password:'Ga0Yl9GNiIfKJDjfj468EWd&8Ei43kx3E4lUl2',
		appToken:'WeaczXHp09DB2p8qg2DzP5onP'
...
function sQuery(params, callback){
	...
	test.query()
		...
		.getRows()
			.on('success', function(rows){callback(rows)})
			.on('error', function(error){callback(error)})
}
