...
visitor.event('application',pkg.name,pkg.version).send();
visitor.event('module',modu.name, modu.version).send();
// *******************************************
...
function entityInfo(options){
	...
	tick.start()
	visitor.event('information',options.type,options.id).send()
	var options = {
        method: 'GET'
        ,uri: (options.type=='company')?config.get("url")+config.get("supplement.data." + options.type)+"?$where=amgpo_making_payment_id=%27"+options.id+"%27":config.get("url")+config.get("supplement.data." + options.type)+config.get("supplement.params."+options.type)+"'"+options.id+"'"
        ,gzip: true
        ,json: true
        ,auth: {
	...
	visitor.timing('information',options.type,myTimer.duration()/1000000,options.id).send()
}
// *******************************************
...
function main(options){
	...
	tick.start()
	visitor.event('summary',options.type,options.id).send()
	...
	visitor.timing('summary',options.type,myTimer.duration()/1000000,options.id).send()
}
// *******************************************
...
function getData(options){
	...
	visitor.event('records',options.type,options.id,counter).send();
	tick.stop()
	var myTimer = t.timers.getData;
	visitor.timing('records',options.type,myTimer.duration()/1000000,counter).send()
}
// *******************************************
